...
import { ChevronDown, ArrowLeft, FileDown, Plus, User, MessageSquare, Clock, CreditCard, Users } from "lucide-react";
...
export default function CompteClient() {
  ...
  // Initialize notes when demande loads
  ...
  if (!demande) return <div className="flex items-center justify-center py-20 text-muted-foreground">Demande introuvable</div>;
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2 pt-4 border-t">
        <Button variant="outline" size="sm" onClick={() => updateMutation.mutate({ statut: "cloturee" })}>Clôturer</Button>
        <Button variant="outline" size="sm" onClick={() => updateMutation.mutate({ statut: "standby" })}>Standby</Button>
        <Button variant="outline" size="sm" className="text-destructive" onClick={() => updateMutation.mutate({ statut: "annulee" })}>Supprimer</Button>
        <Button variant="outline" size="sm" className="text-destructive" onClick={() => updateMutation.mutate({ statut: "rejetee" })}>Rejeter</Button>
      </div>
    </div>
  );
}
